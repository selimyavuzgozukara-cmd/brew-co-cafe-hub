  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center gap-3 flex-wrap">
        ...
        <Button onClick={() => setEditing({ is_active: true })} className="bg-primary hover:bg-primary/90"><Plus size={14} className="mr-1" /> Add product</Button>
      </div>
      ...
      <div className="rounded-xl border border-border bg-card overflow-hidden">
        <table className="w-full text-sm">
          <thead className="bg-muted/50 text-left">
            <tr><th className="p-3">Name</th><th>Category</th><th>Price</th><th>Stock</th><th>Status</th><th></th></tr>
          </thead>
          <tbody>
            {filtered.map(p => (
              <tr key={p.id} className="border-t border-border">
                ...
                <td>{p.is_active ? <span className="text-emerald-700">Active</span> : <span className="text-muted-foreground">Inactive</span>}</td>
      ...
      <Sheet open={!!editing} onOpenChange={(v)=>!v && setEditing(null)}>
        <SheetContent className="w-full sm:max-w-md overflow-y-auto">
          <SheetHeader><SheetTitle className="font-serif text-2xl">{editing?.id ? "Edit" : "New"} product</SheetTitle></SheetHeader>
          {editing && (
      ...
      <AlertDialog open={!!del} onOpenChange={(v)=>!v && setDel(null)}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter><AlertDialogCancel>Cancel</AlertDialogCancel><AlertDialogAction onClick={remove} className="bg-destructive text-destructive-foreground">Delete</AlertDialogAction></AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </div>
  );
